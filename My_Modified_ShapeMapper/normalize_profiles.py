...
def find_boxplot_factor(array):
    ...
    if x.shape[0] < 10:
        ...
        ten_limit = x[x.shape[0] - 1 - ten_pct] # 90%
        five_limit = x[x.shape[0] - 1 - five_pct] # 95%
...
def find_boxplot_factor_fourbases(seq, array):
    seq = seq.replace('U', 'T')
    seq_array = np.array(list(seq))
    A_profile = array[ seq_array=='A' ]
    T_profile = array[ seq_array=='T' ]
    G_profile = array[ seq_array=='G' ]
    C_profile = array[ seq_array=='C' ]
    
    A_fac = find_boxplot_factor(A_profile)
    T_fac = find_boxplot_factor(T_profile)
    G_fac = find_boxplot_factor(G_profile)
    C_fac = find_boxplot_factor(C_profile)
    
    return (A_fac, T_fac, G_fac, C_fac)

def calc_norm_factor(profiles):
    combined = np.hstack(profiles)
    return find_boxplot_factor(combined)

def calc_norm_factor_fourbases(seq_list, profiles):
    seq_comb = "".join(seq_list)
    combined = np.hstack(profiles)
    if len(seq_comb) != len(combined):
        raise RuntimeError("Difference length for combined sequence and profiles")
    return find_boxplot_factor_fourbases(seq_comb, combined)

def normalize_profile(profile, stderrs, sequence, norm_factors):
    sequence = sequence.replace('U', 'T')
    seq_array = np.array(list(sequence))
    nfac = np.zeros(len(sequence))
    nfac[ seq_array=='A' ] = norm_factors[0]
    nfac[ seq_array=='T' ] = norm_factors[1]
    nfac[ seq_array=='G' ] = norm_factors[2]
    nfac[ seq_array=='C' ] = norm_factors[3]
    nfac[ seq_array=='N' ] = np.mean(norm_factors)
        
    norm_profile = profile/nfac
    norm_stderrs = stderrs/nfac
...
def load_profile(filename):
    ...
    for i in range(length):
        line = f.readline()
        s = line.strip().split('\t')
        base = s[seq_col].upper().replace('U', 'T')
        if base not in ('A', 'T', 'C', 'G', 'N'):
            raise RuntimeError("A unknown base "+base+" in "+filename);
        sequence += base
        profile[i] = float(s[profile_col])
        stderrs[i] = float(s[stderr_col])

    return sequence, profile, stderrs
...
if __name__ == "__main__":
    ...
    h = "Normalization with four bases seperately."
    parser.add_argument("--four-bases", action="store_true", default=False, help=h)
    
    h = "Do not normalize 3 bases in both end of sequence."
    parser.add_argument("--disable-end3", action="store_true", default=False, help=h)
    ...
    try:
        if p.four_bases:
            factors = calc_norm_factor_fourbases(seq_list, profile_list)
            print("calculated normalization factors: A:{} T:{} G:{} C:{}".format(factors[0], factors[1], factors[2], factors[3]))
        else:
            factors = calc_norm_factor(profile_list)
            print("calculated normalization factors: {}".format(factors))
    ...
    for i in range(len(p.toscale)):
        ...
        if factors is not None:
            profile, stderr = normalize_profile(profile, stderr, sequence, factors)
            if p.disable_end3:
                profile[0] = profile[1] = profile[2] = nan
                profile[-3] = profile[-2] = profile[-1] = nan
                stderr[0] = stderr[1] = stderr[2] = nan
                stderr[-3] = stderr[-2] = stderr[-1] = nan
