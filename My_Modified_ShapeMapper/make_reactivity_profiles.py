...
def load_counts(filename, Param):
    if filename is None:
        return None
    ...
    if Param.disable_mulmm:
        del mismatch_headers[ mismatch_headers.index("multinuc_mismatch") ]
    if Param.disable_nucmm:
        del mismatch_headers[ mismatch_headers.index("AT") ]
        del mismatch_headers[ mismatch_headers.index("AG") ]
        del mismatch_headers[ mismatch_headers.index("AC") ]
        
        del mismatch_headers[ mismatch_headers.index("TA") ]
        del mismatch_headers[ mismatch_headers.index("TG") ]
        del mismatch_headers[ mismatch_headers.index("TC") ]
        
        del mismatch_headers[ mismatch_headers.index("GA") ]
        del mismatch_headers[ mismatch_headers.index("GT") ]
        del mismatch_headers[ mismatch_headers.index("GC") ]
        
        del mismatch_headers[ mismatch_headers.index("CA") ]
        del mismatch_headers[ mismatch_headers.index("CT") ]
        del mismatch_headers[ mismatch_headers.index("CG") ]
    ...
    ######## Deletion can be disabled
    if Param.disable_muldel:
        del deletion_headers[ deletion_headers.index("multinuc_deletion") ]
    if Param.disable_comdel:
        del deletion_headers[ deletion_headers.index("complex_deletion") ]
    if Param.disable_nucdel:
        del deletion_headers[ deletion_headers.index("A-") ]
        del deletion_headers[ deletion_headers.index("T-") ]
        del deletion_headers[ deletion_headers.index("C-") ]
        del deletion_headers[ deletion_headers.index("G-") ]
    ...
    if Param.disable_mulins:
        del insert_headers[ insert_headers.index("multinuc_insertion") ]
    if Param.disable_comins:
        del insert_headers[ insert_headers.index("complex_insertion") ]
    if Param.disable_nucins:
        del insert_headers[ insert_headers.index("-A") ]
        del insert_headers[ insert_headers.index("-T") ]
        del insert_headers[ insert_headers.index("-C") ]
        del insert_headers[ insert_headers.index("-G") ]
...
if __name__ == "__main__":
    ...
    h = "Disabled multinuc mismatches."
    parser.add_argument("--disable-mulmm", action="store_true", default=False, help=h)
    h = "Disabled single necleotide mismatches."
    parser.add_argument("--disable-nucmm", action="store_true", default=False, help=h)
    h = "Disabled multinuc deletion."
    parser.add_argument("--disable-muldel", action="store_true", default=False, help=h)
    h = "Disabled complex deletion."
    parser.add_argument("--disable-comdel", action="store_true", default=False, help=h)
    h = "Disabled single necleotide deletion."
    parser.add_argument("--disable-nucdel", action="store_true", default=False, help=h)
    h = "Disabled multinuc insertion."
    parser.add_argument("--disable-mulins", action="store_true", default=False, help=h)
    h = "Disabled complex insertion."
    parser.add_argument("--disable-comins", action="store_true", default=False, help=h)
    h = "Disabled single necleotide insertion."
    parser.add_argument("--disable-nucins", action="store_true", default=False, help=h)
    ...
    for i in range(len(samples)):
        try:
            ...
            addtl_column_names = load_counts(p.counts[i], p)
